    ...
    <ScrollView style={styles.screen} contentContainerStyle={styles.scroll}>
      ...
      <View style={styles.moodRow}>
        {[{ id: 'great', emoji: '😊', label: 'Ótimo' }, { id: 'good', emoji: '🙂', label: 'Bom' }, { id: 'regular', emoji: '😐', label: 'Regular' }, { id: 'bad', emoji: '😟', label: 'Ruim' }].map((m) => (
          <TouchableOpacity key={m.id} style={[styles.moodPill, mood === m.id && styles.moodPillActive]} onPress={() => setMood(m.id)}>
      ...
      <View style={styles.actionRow}>
        {[{ id: 'increase', label: 'Aumentar', icon: 'trending-up', color: COLORS.primary }, { id: 'maintain', label: 'Continuar', icon: 'equal', color: COLORS.info }, { id: 'decrease', label: 'Reduzir', icon: 'trending-down', color: COLORS.warning }].map((a) => (
          <TouchableOpacity key={a.id} style={[styles.actionCard, action === a.id && { borderColor: a.color, backgroundColor: a.color + '10' }]} onPress={() => setAction(a.id)}>
      ...
      <TouchableOpacity style={[styles.button, (!mood || !action) && styles.buttonDisabled]} onPress={() => router.replace('/(tabs)/home')} disabled={!mood || !action}>
