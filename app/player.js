...
import { WorkoutTimer, ExerciseProgress, RestOverlay, WorkoutControls, RatingModal } from '../src/components';
...
export default function PlayerScreen() {
  ...
  const handleWorkoutComplete = async () => { gamRef.current = await saveCompleteWorkout(user?.id, workout, timer.logs, timer.elapsed); setShowRating(true); };
              ...
              <View key={i} style={styles.exerciseItem}>
                ...
                <View style={styles.exerciseInfo}>
                  ...
                  <Text style={typography.caption}>
                    {ex.sets || 4}x{ex.reps || 10} · {ex.rest || 60}s descanso
                  </Text>
                </View>
              </View>
            ))}
    ...
    <View style={layout.screen}>
      {timer.phase === 'resting' && (
        <RestOverlay
          ...
          nextSet={timer.currentSet}
          nextIndex={timer.currentExerciseIndex}
