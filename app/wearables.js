...
export default function WearablesScreen() {
  ...
  const init = useCallback(async () => {
    const [w, h, s] = await Promise.all([checkWatchAvailability(), checkHealthAvailability(), checkStravaAvailability()]);
    setWAvail(w.available); setHAvail(h.available); setSAvail(s.available);
    setWConn(isWatchConnected()); setSConn(isStravaConnected());
    if (h.available) { const p = await requestHealthPermissions(); setHConn(p.granted); }
    const [hr, st, cal] = await Promise.all([getWatchHeartRate(), getStepsToday(), getCaloriesBurnedToday()]);
    if (hr) { setHeartRate(hr); setRestingHR(62); setMaxHR(185); }
    setSteps(st); setCalories(cal); setDistance(st * 0.0007);
    ...
    <ErrorBoundary screenName="Wearables">
      <View style={s.screen}>
        ...
        <ScrollView style={s.scroll} contentContainerStyle={s.scrollContent} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={COLORS.primary} />} showsVerticalScrollIndicator={false}>
          ...
          <ActivitySummary steps={steps} stepsGoal={8000} calories={calories} caloriesGoal={500} distance={distance} />
        </ScrollView>
      </View>
    </ErrorBoundary>
  );
