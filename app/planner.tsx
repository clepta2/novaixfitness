...
import { ErrorBoundary, WeekCalendar, AdaptationBanner, DayEditorModal, DayDetailView, TodayCard, WeekOverview } from '../src/components';
...
export default function PlannerScreen() {
  ...
  useEffect(() => {
    async function loadPlan() {
      ...
      } catch (e) { if (__DEV__) console.warn('PlannerScreen:', e); }
    }
    loadPlan();
  }, [user?.id]);
  ...
  const handleEditDay = useCallback(async (dayData) => {
    ...
    const result = await call(() => updateDayPlan(user.id, editingDay, dayData));
    if (result.ok) {
  ...
  return (
    <ErrorBoundary screenName="Planner">
