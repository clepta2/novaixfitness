...
import { HistoryCard, ErrorBoundary } from '../../../src/components';
    ...
    <ErrorBoundary screenName="History">
      <View style={layout.screen}>
        <View style={layout.header}>
          <TouchableOpacity accessibilityLabel="Voltar" accessibilityRole="button" onPress={() => router.back()} style={layout.headerBtn}>
            ...
            {workouts.length > 0 && (
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.filterScroll}>
                {HISTORY_FILTERS.map((f) => (
                  <TouchableOpacity key={f.key} accessibilityLabel={`Filtrar ${f.label}`} accessibilityRole="button" style={[styles.filterBtn, filter === f.key && styles.filterBtnActive]} onPress={() => setFilter(f.key)}>
