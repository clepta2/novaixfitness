...
export default function NotificationsScreen() {
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    try {
      ...
      if (__DEV__) console.error('Erro ao buscar notificações:', err);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
    ...
    <View style={layout.screen}>
      ...
      {notifications.length === 0 && !loading ? (
        <View style={styles.empty}>
          ...
          <Text style={typography.bodyMuted}>Suas notificações aparecerão aqui</Text>
        </View>
      ) : (
        <FlatList
          data={notifications}
          ...
          renderItem={({ item }) => (
            <NotificationItem item={item} onPress={() => handleNotificationPress(item)} onLongPress={() => markAsRead(item.id)} />
          )}
