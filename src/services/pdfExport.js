...
export async function exportMealPlanToPDF(mealPlan) {
...
export async function exportWorkoutPlanToPDF(workoutPlan) {
...
function generateMealPlanHTML(plan) {
  const daysHTML = plan.week.map(day => `
    <div class="day-card">
      <h3>${day.day}</h3>
  ...
  const tipsHTML = plan.summary?.tips?.map(t => `<li>${t}</li>`).join('') || '';
...
function generateWorkoutPlanHTML(plan) {
  const daysHTML = plan.week.map(day => `
    <div class="day-card">
