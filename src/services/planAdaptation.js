...
export async function analyzeUserPerformance(userId) {
  ...
  } catch (err) { console.error('Erro ao analisar performance:', err); return null; }
}
...
function calculateConsistency(workouts) {
  ...
  const weeksSpan = Math.max(1, (Date.now() - new Date(Math.min(...workouts.map(w => new Date(w.completed_at || w.created_at))))) / (7 * 86400000));
...
export async function saveAdaptation(userId, oldPlan, newPlan, reason) {
  ...
  } catch (err) { console.error('Erro ao salvar adaptação:', err); }
}
