...
export async function exportWorkoutHistory(userId) {
  ...
  try {
    ...
    if (__DEV__) console.error('Erro ao exportar historico:', err);
    throw err;
  }
}
...
export async function exportProgressData(userId) {
  ...
  try {
    ...
    if (__DEV__) console.error('Erro ao exportar progresso:', err);
    throw err;
  }
}
...
export async function exportAnalyticsData(userId, period = 'month') {
  ...
  try {
    ...
    if (__DEV__) console.error('Erro ao exportar analytics:', err);
    throw err;
  }
}
...
export async function exportAchievements(userId) {
  ...
  try {
    ...
    if (__DEV__) console.error('Erro ao exportar conquistas:', err);
    throw err;
  }
}
...
export async function exportProfileData(userId) {
  ...
  try {
    ...
    if (__DEV__) console.error('Erro ao exportar perfil:', err);
    throw err;
  }
}
...
export async function exportChatHistory(userId) {
  ...
  try {
    ...
    if (__DEV__) console.error('Erro ao exportar chat:', err);
    throw err;
  }
}
...
export async function exportAllData(userId) {
  ...
  try {
    ...
    if (__DEV__) console.error('Erro ao exportar todos dados:', err);
    throw err;
  }
}
...
export async function deleteAllUserData(userId) {
  if (!userId) throw new Error('Usuario nao autenticado');
  try {
    const tables = ['coach_chat_messages', 'user_achievements', 'favorites', 'post_likes', 'post_comments', 'posts', 'user_workouts', 'weight_logs', 'water_logs', 'injury_details'];
    await Promise.all(tables.map(t => supabase.from(t).delete().eq('user_id', userId)));
    await supabase.from('profiles').update({
      name: 'Conta excluida', email: `deleted_${userId}@novaix.fitness`,
      cpf: null, phone: null, onboarding: null, subscription_status: 'cancelled',
    }).eq('id', userId);
  } catch (err) {
    if (__DEV__) console.error('Erro ao deletar dados:', err);
    throw err;
  }
}
