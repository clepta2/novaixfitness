...
export async function applyCoupon(code, planId) {
  ...
  if (coupon.applies_to && coupon.applies_to !== planId && coupon.applies_to !== 'all') {
    return null;
  }
...
export async function createReferral(userId) {
...
export async function applyReferral(referralCode, newUserId) {
  ...
  await supabase
    ...
    .insert({
      ...
      title: 'Recompensa por indicacao!',
      body: 'Voce ganhou 7 dias gratis por indicar um amigo!',
      data: { days: 7 },
    });
