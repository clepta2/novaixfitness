...
export async function getUserChallenges(userId: string) {
  const { data } = await supabase.from('friend_challenges')
...
export async function getChallengeHistory(userId: string) {
  const { data } = await supabase.from('challenge_history')
...
export async function getWinRate(userId: string) {
  const { count: wins } = await supabase.from('challenge_history')
    .select('id', { count: 'exact', head: true }).eq('winner_id', userId);
  const { count: losses } = await supabase.from('challenge_history')
