...
export async function createSubscription(userId: string, planId: string, paymentMethod: string): Promise<Subscription> {
  ...
  trackEvent((EVENT_TYPES as any).SUBSCRIPTION_STARTED, { plan_id: planId, price: plan.price, user_id: userId });
...
export async function cancelSubscription(userId: string): Promise<void> {
  ...
  trackEvent((EVENT_TYPES as any).SUBSCRIPTION_CANCELLED, { user_id: userId });
}
