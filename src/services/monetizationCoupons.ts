...
export async function recordCouponUsage(couponCode: string, userId: string): Promise<void> {
  await supabase.from('coupon_usage').insert({ coupon_code: couponCode, user_id: userId });
...
export async function createReferral(userId: string): Promise<string> {
  ...
  await supabase.from('referrals').upsert({ user_id: userId, code, referral_count: 0 }, { onConflict: 'user_id' });
  return code;
}
...
export async function getReferralStats(userId: string): Promise<{ code: string | null; referral_count: number }> {
  const { data } = await supabase.from('referrals').select('*').eq('user_id', userId).single();
