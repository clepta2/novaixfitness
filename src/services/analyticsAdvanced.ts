// src/services/analyticsAdvanced.js
// Cohort analysis, funis, segmentacao de usuarios

import { supabase } from '../config/supabase';
import { TABLES } from '../config/tables';
import { createServiceGuard } from '../utils/serviceGuard';

const guard = createServiceGuard({ serviceName: 'analyticsAdvanced' });
  ...
  const result = await guard.guard(async () => {
    ...
    return {
      ...
      stats: {
        ...
        basic: segments.basic.length,
        premium: segments.premium.length,
        powerUsers: segments.power_users.length,
        newUsers: segments.new_users.length,
      },
    };
  });
