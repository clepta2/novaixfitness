...
export async function getCustomBannedWords() {
  const { data } = await supabase.from('moderation_custom_words').select('*').eq('is_active', true);
  return data || [];
}

export async function addCustomBannedWord(word, category, addedBy) {
  const result = await guard.guard(async () => {
    const { error } = await supabase.from('moderation_custom_words').insert({
...
export async function removeCustomBannedWord(wordId) {
  const result = await guard.guard(async () => {
    const { error } = await supabase.from('moderation_custom_words').delete().eq('id', wordId);
    if (error) throw error;
    return true;
  });
  return result.ok ? result.data : false;
...
export async function getModerationStats() {
  const [blocksResult, flagsResult, trustResult] = await Promise.all([
    supabase.from('user_blocks').select('id', { count: 'exact', head: true }).eq('is_active', true),
    supabase.from('content_flags').select('id', { count: 'exact', head: true }).eq('status', 'pending'),
    supabase.from('user_trust').select('trust_score').lte('trust_score', 30),
  ]);
