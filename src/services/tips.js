...
export async function generatePersonalizedTips(profileContext) {
  const apiKey = process.env.EXPO_PUBLIC_GOOGLE_API_KEY || '';
  
  const systemPrompt = `Gere 5 dicas personalizadas para o usuário em JSON válido.

Perfil:
- Objetivo: ${profileContext.goal}
- Nível: ${profileContext.level}
- Peso: ${profileContext.weight}kg
- Idade: ${profileContext.ageRange}
...
function generateFallbackTips(profile) {
  return {
    nutrition: [
