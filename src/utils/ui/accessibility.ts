...
export async function isScreenReaderEnabled() {
  const result = await tryIf(() => AccessibilityInfo.isScreenReaderEnabled(), { retries: 1, baseDelay: 500 });
  return result.ok ? result.data! : false;
}

// Check if reduce motion is enabled
export async function isReduceMotionEnabled() {
  const result = await tryIf(() => AccessibilityInfo.isReduceMotionEnabled(), { retries: 1, baseDelay: 500 });
  return result.ok ? result.data! : false;
}
...
// Suggest accessible color based on background luminance
export function suggestAccessibleColor(background: string): string {
  const rgb = background.replace('#', '').match(/.{2}/g)!.map(x => {
    const val = parseInt(x, 16) / 255;
    return val <= 0.03928 ? val / 12.92 : Math.pow((val + 0.055) / 1.055, 2.4);
  });
