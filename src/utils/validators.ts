...
// Validacao centralizada de inputs antes de chamadas async - NOVAIX FITNESS
...
export const Rules = {
  ...
  cpf: (): ValidationRule<string> => ({
    test: (v) => /^\d{11}$/.test(v.replace(/\D/g, '')),
...
// Valida multiplos campos de uma vez
