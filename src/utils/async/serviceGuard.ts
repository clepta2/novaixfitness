...
export function createServiceGuard(options: ServiceOptions) {
  ...
  return {
    ...
    guardWithRetry: async <T>(
      ...
      const result = await tryIf(fn, {
        retries: maxRetries,
        baseDelay: delayMs,
        maxDelay: 30000,
        onRetry: (error, attempt) => {
          if (log) console.warn(`[${serviceName}] tentativa ${attempt} falhou:`, error.message);
        },
        onFail: (error) => {
          if (log) console.error(`[${serviceName}] falhou apos ${maxRetries + 1} tentativas:`, error.message);
          trackError(error, `${serviceName}.retry`, 'high', { maxRetries });
        },
      });

      return result.ok
        ? { ok: true, data: result.data! }
        : { ok: false, error: result.error?.message || 'Erro desconhecido' };
    },
