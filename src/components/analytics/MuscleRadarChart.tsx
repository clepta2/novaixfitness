...
function polarToCartesian(index: number, value: number, total: number) {
  const angle = (Math.PI * 2 * index) / total - Math.PI / 2;
  return { x: CENTER + (value / 100) * RADIUS * Math.cos(angle), y: CENTER + (value / 100) * RADIUS * Math.sin(angle) };
}

function getTrend(current: number, previous: number | null): { icon: string; color: string; text: string } | null {
...
function getBalanceScore(values: { current: number }[]) {
...
function getInsight(values: { current: number; label: string }[]) {
...
export default function MuscleRadarChart({ data = {}, previousData = null }: MuscleRadarChartProps): React.JSX.Element {
