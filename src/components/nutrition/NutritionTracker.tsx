...
function MacroBar({ label, current, goal, color, icon, delay = 0 }: MacroBarProps): React.JSX.Element {
...
export default function NutritionTracker({ userId, weight = 70, goal = 'manter' }: NutritionTrackerProps): React.JSX.Element {
  const [summary, setSummary] = useState<DailySummary | null>(null);
  const [loading, setLoading] = useState<boolean>(true);
  const calorieAnim = useRef(new Animated.Value(0)).current;

  const goals = useMemo((): NutritionGoals => calculateNutritionGoals(weight, goal), [weight, goal]);
  ...
  const data: DailySummary = summary || { calories: 0, protein: 0, carbs: 0, fat: 0, fiber: 0, meals: 0 };
