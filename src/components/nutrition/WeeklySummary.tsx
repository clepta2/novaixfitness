// src/components/nutrition/WeeklySummary.tsx
...
function StatBar({ label, value, avg, max, color, unit = '' }: StatBarProps): React.JSX.Element {
...
export default function WeeklySummary({ userId }: WeeklySummaryProps): React.JSX.Element {
  ...
  const loadWeek = async (): Promise<void> => {
    ...
    try {
      ...
      const weekLogs = logs.filter((l: MealLog) => new Date(l.logged_at) >= new Date(weekAgo));
      setData(weekLogs);
    } catch { }
