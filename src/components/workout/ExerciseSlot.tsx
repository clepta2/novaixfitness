...
interface ExerciseSlotProps {
  exercise: ExerciseSlotData;
  index: number;
  onRemove: (index: number) => void;
  onUpdate: (index: number, exercise: ExerciseSlotData) => void;
}

export default memo(function ExerciseSlot({ exercise, index, onRemove, onUpdate }: ExerciseSlotProps): React.ReactElement {
    ...
    <Animated.View style={[styles.container, { transform: [{ scale: scaleAnim }] }]}>
      ...
      <View style={styles.inputs}>
        <View style={styles.inputGroup}>
          ...
          <TextInput style={styles.miniInput} value={String(exercise.sets || 4)} onChangeText={(v: string) => onUpdate(index, { ...exercise, sets: parseInt(v) || 4 })} keyboardType="numeric" />
        </View>
        <View style={styles.inputGroup}>
          <Text style={styles.inputLabel}>REPS</Text>
          <TextInput style={styles.miniInput} value={String(exercise.reps || '10-12')} onChangeText={(v: string) => onUpdate(index, { ...exercise, reps: v })} />
        </View>
        <View style={styles.inputGroup}>
          <Text style={styles.inputLabel}>DESCANSO</Text>
          <TextInput style={styles.miniInput} value={String(exercise.rest || 60)} onChangeText={(v: string) => onUpdate(index, { ...exercise, rest: parseInt(v) || 60 })} keyboardType="numeric" />
        </View>
      </View>
