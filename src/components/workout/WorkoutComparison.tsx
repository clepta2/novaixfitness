// src/components/workout/WorkoutComparison.tsx
...
function ComparisonRow({ label, current, previous, unit = '', better = 'higher' }: ComparisonRowProps): React.ReactElement | null {
...
export default function WorkoutComparison({ current, previous }: WorkoutComparisonProps): React.ReactElement | null {
  if (!previous) return null;
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.card}>
        <ComparisonRow label="Duração" current={current.duration || 0} previous={previous.duration ?? 0} unit=" min" />
        <ComparisonRow label="Exercícios" current={current.exercises || 0} previous={previous.exercises ?? 0} />
        <ComparisonRow label="Séries" current={current.sets || 0} previous={previous.sets ?? 0} />
      </View>
    </View>
  );
}
