...
const MUSCLES: ChipItem[] = [{ key: 'all', l: 'Todos' }, { key: 'chest', l: 'Peito' }, { key: 'back', l: 'Costas' }, { key: 'legs', l: 'Pernas' }, { key: 'shoulders', l: 'Ombros' }, { key: 'arms', l: 'Braços' }, { key: 'abs', l: 'Abdômen' }, { key: 'cardio', l: 'Cardio' }];
const EQUIP: ChipItem[] = [{ key: 'all', l: 'Todos' }, { key: 'none', l: 'Corporal' }, { key: 'barbell', l: 'Barra' }, { key: 'dumbbells', l: 'Halteres' }, { key: 'machine', l: 'Máquina' }, { key: 'cable', l: 'Cabo' }];
const MC: Record<string, string> = { chest: COLORS.success, back: COLORS.info, legs: COLORS.secondary, shoulders: COLORS.attention, arms: COLORS.primary, abs: COLORS.success, cardio: COLORS.rose };

function flatten(muscleFilter: string): ExerciseItem[] {
  const list: ExerciseItem[] = [];
  (muscleFilter === 'all' ? Object.keys(EXERCISE_CATALOG) : [muscleFilter]).forEach(g => {
    const lv = EXERCISE_CATALOG[g] || {};
    ['beginner', 'intermediate', 'advanced'].forEach(l => (lv[l] || []).forEach((ex: ExerciseItem) => list.push({ ...ex, muscleGroup: g, level: l })));
  });
  return list;
}

const Chips = ({ data, active, onSelect }: { data: ChipItem[]; active: string; onSelect: (key: string) => void }) => (
...
export default function ExerciseSearch({ onSelect, selectedIds = [] }: ExerciseSearchProps): React.ReactElement {
