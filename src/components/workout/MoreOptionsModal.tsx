// src/components/workout/MoreOptionsModal.tsx
...
function OptionItemComponent({ option, index, onPress }: OptionItemProps): React.ReactElement {
...
function MoreOptionsModal({ visible, onSelect, onClose }: MoreOptionsModalProps): React.ReactElement {
      ...
      <TouchableOpacity style={styles.overlay} activeOpacity={1} onPress={onClose}>
        <Animated.View style={[styles.content, { transform: [{ translateY: slideAnim }] }]} onStartShouldSetResponder={() => true}>
          ...
          <View style={styles.optionsList}>
            {OPTIONS.map((opt, i) => (
              <OptionItemComponent key={opt.key} option={opt} index={i} onPress={onSelect} />
            ))}
          </View>
