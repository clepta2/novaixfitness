...
function WaterGlass({ filled }: WaterGlassProps): React.JSX.Element {
  return (
...
export default memo(function WaterLogger(): React.JSX.Element {
  const { user } = useAuth();
  ...
  const loadWater = useCallback(async () => {
    try {
      ...
      if (user?.id) {
        ...
        if (data) {
          const total = data.reduce((sum: number, item: any) => sum + item.amount_ml, 0);
          setConsumed(total);
