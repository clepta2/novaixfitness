...
interface ContextualCardProps {
  card: ContextualCardData;
  onAction?: () => void;
  streak?: number;
}

function getStreakMotivation(streak: number): { text: string; icon: string; color: string } | null {
...
export default function ContextualCard({ card, onAction, streak = 0 }: ContextualCardProps): React.JSX.Element {
    ...
    <Animated.View style={[styles.card, { backgroundColor: COLORS.surface, opacity: fadeAnim, transform: [{ translateY: slideAnim }, { scale: pulseAnim }] }]}>
      <View style={styles.header}>
        <View style={[styles.iconWrap, { backgroundColor: card.iconColor + '20' }]}>
          <Ionicons name={card.icon as any} size={scale(24)} color={card.iconColor} />
        </View>
