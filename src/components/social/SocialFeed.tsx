...
import { RepostCard } from '../exports/social';
...
function filterPosts(posts: ActivityData[], filter: string): ActivityData[] {
...
export default function SocialFeed({ userId }: SocialFeedProps) {
  ...
  const loadActivities = async () => {
    ...
    try {
      ...
      const formatted: ActivityData[] = workouts.map((w: any) => ({
    ...
    <View style={styles.container}>
      ...
      {filtered.length === 0 ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            if (item.type === 'repost') return <RepostCard originalPost={item.originalPost} repostedBy={item.repostedBy} />;
