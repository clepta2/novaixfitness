...
function AnimatedProgressBar({ progress, target, color }: AnimatedProgressBarProps) {
...
interface ChallengeCardProps {
  ...
  index: number;
  onPress?: (challenge: ChallengeData) => void;
  onShare?: (challenge: ChallengeData) => void;
}

function ChallengeCard({ challenge, index, onPress, onShare }: ChallengeCardProps) {
  const animatedValue = useMemo(() => new Animated.Value(0), []);
      ...
      <TouchableOpacity style={styles.cardContent} onPress={() => onPress?.(challenge)} activeOpacity={0.8}>
        <View style={styles.cardHeader}>
          <View style={[styles.iconContainer, { backgroundColor: color + '20' }]}>
            <Ionicons name={isComplete ? 'checkmark-circle' : (icon as any)} size={22} color={isComplete ? COLORS.success : color} />
          </View>
