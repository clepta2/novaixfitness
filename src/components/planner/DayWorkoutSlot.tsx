// src/components/planner/DayWorkoutSlot.tsx
...
interface DayWorkoutSlotProps {
  dayData: DayData | null;
  dayName: string;
  dateNumber: number;
  isToday: boolean;
  onPress?: (data: DayData | null) => void;
}

export default function DayWorkoutSlot({ dayData, dayName, dateNumber, isToday, onPress }: DayWorkoutSlotProps): React.JSX.Element {
  const isRest = dayData?.isRest;
  const catColor = CATEGORY_COLORS[dayData?.category as keyof typeof CATEGORY_COLORS] || COLORS.primary;
      ...
      <View style={[styles.slot, isRest ? styles.restSlot : styles.workoutSlot, isToday && styles.slotToday]}>
        {isRest ? (
          ...
          <>
            ...
            {dayData?.duration && dayData.duration > 0 && (
