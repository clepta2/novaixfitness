// src/components/planner/DayEditorModal.tsx
...
interface DayEditorModalProps {
  ...
  onSave: (data: CurrentData) => void;
  onClose: () => void;
}

export default function DayEditorModal({ visible, dayKey, currentData, onSave, onClose }: DayEditorModalProps): React.JSX.Element {
  const [selectedWorkout, setSelectedWorkout] = useState<{ id: string; title: string; category: string; duration_minutes?: number } | null>(
    currentData?.isRest ? null : { id: currentData?.workoutId || '', title: currentData?.workoutName || '', category: currentData?.category || '', duration_minutes: currentData?.duration }
  );
  ...
  const handleSave = (): void => {
    if (!selectedWorkout) {
      onSave({ isRest: true, workoutId: null as unknown as string, workoutName: null as unknown as string, duration: 0, category: null as unknown as string });
    } else {
      onSave({
        isRest: false,
    ...
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      <View style={styles.overlay}>
        <View style={styles.modal}>
          ...
          <WorkoutPicker selectedId={selectedWorkout?.id ?? null} onSelect={(w) => setSelectedWorkout(w ? { id: w.id, title: w.title, category: w.category, duration_minutes: w.duration_minutes } : null)} />
