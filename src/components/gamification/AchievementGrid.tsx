...
function AchievementBadge({ achievement, unlocked, onInfo }: { achievement: Achievement; unlocked: boolean; onInfo: (a: Achievement) => void }): React.JSX.Element {
  ...
  return (
    <TouchableOpacity style={[styles.badge, !unlocked && styles.badgeLocked]} onPress={() => onInfo(achievement)} accessibilityLabel={`${achievement.name} - ${unlocked ? 'Desbloqueada' : 'Bloqueada'}`} accessibilityRole="button">
      <View style={[styles.iconWrap, { backgroundColor: color + '20' }]}>
        <Ionicons name={unlocked ? achievement.icon as any : 'lock-closed'} size={ICON_SIZES.md} color={color} />
      </View>
...
function AchievementGrid({ unlockedIds = [], onAchievementPress }: AchievementGridProps): React.JSX.Element {
  const [activeCategory, setActiveCategory] = useState<string>('all');
  const [selectedInfo, setSelectedInfo] = useState<Achievement | null>(null);
  const filtered = activeCategory === 'all' ? ACHIEVEMENTS : ACHIEVEMENTS.filter(a => a.category === activeCategory);
  const unlockedSet = new Set(unlockedIds);
  const handleInfo = (a: Achievement): void => onAchievementPress ? onAchievementPress(a) : setSelectedInfo(selectedInfo?.id === a.id ? null : a);
      ...
      {selectedInfo && (
        <View style={styles.infoCard}>
          <View style={[styles.infoIcon, { backgroundColor: selectedInfo.color + '20' }]}>
            <Ionicons name={selectedInfo.icon as any} size={24} color={selectedInfo.color} />
          </View>
