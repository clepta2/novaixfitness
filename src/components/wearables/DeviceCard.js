...
const ICONS = { watch: 'watch-outline', health: 'heart-outline', strava: 'logo-strava' };

export default function DeviceCard({ device, onToggle, onSync, syncing }) {
    ...
    <View style={styles.card} accessibilityLabel={`${name}: ${connected ? 'conectado' : 'desconectado'}`}>
      <View style={styles.row}>
        <View style={[styles.iconWrap, connected && styles.iconConnected]}>
          <Ionicons name={ICONS[type] || 'hardware-chip-outline'} size={24} color={connected ? COLORS.primary : COLORS.textMuted} />
        </View>
