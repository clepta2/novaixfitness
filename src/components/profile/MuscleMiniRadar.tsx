...
interface MuscleMiniRadarProps {
  userId: string;
}
...
export default function MuscleMiniRadar({ userId }: MuscleMiniRadarProps): React.JSX.Element | null {
  ...
  useEffect(() => {
    ...
    async function load(): Promise<void> {
      try {
        ...
        const processWorkouts = (workouts: any[]): Record<string, number> => {
        ...
        const balanced: MuscleData[] = MUSCLES.map((m: any) => {
          const current = currentCounts[m.key] || 0;
          const prev = prevCounts[m.key] || 0;
          const value = Math.round((current / max) * 100);
          const trend: MuscleData['trend'] = current > prev ? 'up' : current < prev ? 'down' : 'stable';
        ...
        balanced.forEach((m: MuscleData, i: number) => {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.bars}>
        {data.map((m: MuscleData, i: number) => (
