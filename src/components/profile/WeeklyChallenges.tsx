...
export default function WeeklyChallenges({ userId, compact = false }: WeeklyChallengesProps): React.JSX.Element {
  const router = useRouter();
  const [progress, setProgress] = useState<ProgressState>({ workouts: 0, minutes: 0, posts: 0, streak: 0 });
  ...
  useEffect(() => {
    async function loadProgress(): Promise<void> {
      try {
        ...
        const totalMinutes = workoutsRes.data?.reduce((s: number, w: any) => s + (w.duration || 0), 0) || 0;
        ...
        const claimedIds = new Set<string>((claimedRes.data || []).map((a: any) => a.achievement_id));
  ...
  const getProgress = (challenge: any): number => {
    let current = 0;
  ...
  const getCurrentValue = (challenge: any): number => {
    switch (challenge.type) {
  ...
  const handleClaim = useCallback(async (challenge: any): Promise<void> => {
    ...
    try {
      ...
      setClaimed((prev: Set<string>) => new Set([...prev, challenge.id]));
  ...
  return (
    <View style={[styles.container, compact && styles.containerCompact]}>
      ...
      {(compact ? WEEKLY_CHALLENGES.slice(0, 3) : WEEKLY_CHALLENGES).map((challenge: any) => {
        const prog = getProgress(challenge);
