...
const STAT_COLORS: Record<string, string> = { streak: COLORS.secondary, workouts: COLORS.primary, time: COLORS.info, favorites: COLORS.error };
...
function StatChip({ icon, color, label, value, unit }: StatChipProps): React.JSX.Element {
  return (
    <View style={styles.chip}>
      <View style={[styles.chipIcon, { backgroundColor: color + '22' }]}>
        <Ionicons name={icon as any} size={ICON_SIZES.sm} color={color} />
      </View>
...
function ProfileHero({ name, email, memberSince, uri, onPressAvatar, onEditName, stats, xp = 0 }: ProfileHeroProps): React.JSX.Element {
