...
export default function DayDetails({ byDay }: DayDetailsProps): React.JSX.Element | null {
  const activeDays = byDay?.filter((d: DayData) => d.count > 0) || [];
  if (activeDays.length === 0) return null;

  return (
    <View style={styles.section}>
      <Text style={typography.label}>DETALHES POR DIA</Text>
      {activeDays.map((day: DayData, i: number) => (
        <View key={i} style={styles.card}>
          <View style={styles.cardHeader}>
            <Text style={typography.h5}>{day.fullDay}</Text>
            <Text style={typography.caption}>{day.count} treinos • {day.minutes} min</Text>
          </View>
          {day.workouts.map((w: Workout, j: number) => (
