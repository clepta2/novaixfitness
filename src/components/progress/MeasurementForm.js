// src/components/progress/MeasurementForm.js
  ...
  return (
    <View style={styles.inputRow}>
      <View style={[styles.inputIcon, { backgroundColor: measurement.color + '15' }]}>
        <Ionicons name={measurement.icon} size={16} color={measurement.color} />
      </View>
    ...
    <Animated.View style={[styles.card, { opacity: fadeAnim }]}>
      ...
      <View style={styles.inputsGrid}>
        {MEASUREMENT_TYPES.map((m) => (
          <MeasurementInput key={m.key} measurement={m} value={form[m.key] || ''} onChange={(v) => onChangeForm({ ...form, [m.key]: v })} />
        ))}
      </View>
