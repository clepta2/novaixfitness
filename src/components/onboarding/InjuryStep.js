// src/components/onboarding/InjuryStep.js
// Step de lesões
...
export default function InjuryStep({ step, data, onUpdate }) {
            ...
            <>
              <View style={styles.injuryGrid}>
                {INJURY_OPTIONS.map((injury) => (
                  <TouchableOpacity key={injury.id} style={[styles.injuryChip, injuries.some(i => i.bodyPart === injury.id) && styles.injuryChipActive]} onPress={() => {
                    if (injury.id === 'none') { onUpdate('injuries', []); }
                    else { setCurrentInjury(injury.id); setShowForm(true); }
                  }}>
                    <Text style={[styles.injuryChipText, injuries.some(i => i.bodyPart === injury.id) && styles.injuryChipTextActive]}>{injury.label}</Text>
                  </TouchableOpacity>
                ))}
              </View>
              {showForm && (
                <View style={styles.injuryForm}>
                  ...
                  <TextInput style={styles.textArea} multiline placeholder="Ex: Machuquei essa semana jogando futebol" placeholderTextColor={COLORS.textMuted} value={injuries.find(i => i.bodyPart === currentInjury)?.description || ''} onChangeText={(t) => {
                    const updated = injuries.filter(i => i.bodyPart !== currentInjury);
