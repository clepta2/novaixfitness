// src/components/onboarding/CepStep.tsx
...
export default function CepStep({ step, data, onUpdate }: CepStepProps): React.JSX.Element {
  const [loading, setLoading] = useState<boolean>(false);
  const handleCepChange = async (text: string): Promise<void> => {
      ...
      {step.fields.map((field) => (
        <View key={field.id} style={styles.inputGroup}>
          ...
          <View style={styles.inputWrap}>
            <TextInput style={styles.textInput} value={data[field.field] || ''} onChangeText={field.type === 'cep_input' ? handleCepChange : (t: string) => onUpdate(field.field, t)} editable={!field.autoFilled || !data[field.field]} placeholder={field.type === 'cep_input' ? '00000-000' : ''} placeholderTextColor={COLORS.textMuted} />
