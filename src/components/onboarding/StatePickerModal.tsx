// src/components/onboarding/StatePickerModal.tsx
...
export default function StatePickerModal({ visible, selectedState, onSelect, onClose }: StatePickerModalProps): React.JSX.Element {
  const [search, setSearch] = useState<string>('');

  const filtered = ESTADOS.filter((e: Estado) =>
    e.nome.toLowerCase().includes(search.toLowerCase()) ||
    e.sigla.toLowerCase().includes(search.toLowerCase())
  );

  const handleSelect = (sigla: string): void => {
    onSelect(sigla);
    setSearch('');
    onClose();
  };
    ...
    <Modal visible={visible} animationType="slide" transparent>
      <TouchableWithoutFeedback onPress={() => { setSearch(''); onClose(); }}>
        <View style={styles.overlay}>
          ...
          windowSize={5}
                data={filtered}
                keyExtractor={(item: Estado) => item.sigla}
