...
export default function AdminDashboard({ onNavigate }: AdminDashboardProps): React.JSX.Element {
  const [stats, setStats] = useState<DashboardStats>({ total: 0, active: 0, newMonth: 0, workouts: 0 });
  ...
  async function loadDashboard(): Promise<void> {
    try {
      ...
      setStats({
        ...
        active: profiles.filter((p: any) => p.subscription_status === 'premium' || p.subscription_status === 'active').length,
        newMonth: profiles.filter((p: any) => p.created_at >= monthStart).length,
        workouts: (workoutsRes.data || []).length,
      });

      setRecentActivity((recentRes.data || []).map((p: any) => ({
        id: p.id,
  ...
  const summaryCards = [
    { key: 'total', label: 'TOTAL ALUNOS', value: stats.total, icon: 'people' as const, color: COLORS.primary },
    { key: 'active', label: 'ATIVOS', value: stats.active, icon: 'checkmark-circle' as const, color: COLORS.success },
    { key: 'newMonth', label: 'NOVOS/MÊS', value: stats.newMonth, icon: 'person-add' as const, color: COLORS.info },
    { key: 'workouts', label: 'TREINOS', value: stats.workouts, icon: 'barbell' as const, color: COLORS.secondary },
  ];

  const quickActions = [
    { key: 'students', label: 'Alunos', icon: 'people' as const, color: COLORS.primary },
    { key: 'finance', label: 'Financeiro', icon: 'cash' as const, color: COLORS.success },
    { key: 'content', label: 'Conteúdo', icon: 'document-text' as const, color: COLORS.info },
  ];
