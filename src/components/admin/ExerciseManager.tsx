...
export default function ExerciseManager(): React.JSX.Element {
  const [exercises, setExercises] = useState<Exercise[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [modalVisible, setModalVisible] = useState<boolean>(false);
  const [editing, setEditing] = useState<Exercise | null>(null);
  const [form, setForm] = useState<ExerciseForm>({ name: '', category: 'Musculacao', muscle_group: '', equipment: '' });
      ...
      <Modal visible={modalVisible} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <TextInput style={styles.input} value={form.name} onChangeText={(t: string) => setForm({ ...form, name: t })} placeholder="Nome do exercício" placeholderTextColor={COLORS.textMuted} accessibilityLabel="Nome do exercício" />
            <TextInput style={styles.input} value={form.category} onChangeText={(t: string) => setForm({ ...form, category: t })} placeholder="Categoria" placeholderTextColor={COLORS.textMuted} accessibilityLabel="Categoria do exercício" />
            <TextInput style={styles.input} value={form.muscle_group} onChangeText={(t: string) => setForm({ ...form, muscle_group: t })} placeholder="Grupo muscular" placeholderTextColor={COLORS.textMuted} accessibilityLabel="Grupo muscular" />
            <TextInput style={styles.input} value={form.equipment} onChangeText={(t: string) => setForm({ ...form, equipment: t })} placeholder="Equipamento" placeholderTextColor={COLORS.textMuted} accessibilityLabel="Equipamento necessário" />
