...
export default function WorkoutManager(): React.JSX.Element {
  const [workouts, setWorkouts] = useState<Workout[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [modalVisible, setModalVisible] = useState<boolean>(false);
  const [editing, setEditing] = useState<Workout | null>(null);
  const [form, setForm] = useState<WorkoutForm>({ title: '', category: 'Musculacao', level: 'Iniciante', duration_minutes: 30, video_id: '', is_premium: false });
  ...
  function openEdit(workout: Workout): void {
    setEditing(workout);
      ...
      <Modal visible={modalVisible} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <TextInput style={styles.input} value={form.title} onChangeText={(t: string) => setForm({ ...form, title: t })} placeholder="Título do treino" placeholderTextColor={COLORS.textMuted} accessibilityLabel="Título do treino" />
            <View style={styles.row}>
              <TextInput style={[styles.input, { flex: 1 }]} value={form.category} onChangeText={(t: string) => setForm({ ...form, category: t })} placeholder="Categoria" placeholderTextColor={COLORS.textMuted} accessibilityLabel="Categoria" />
              <TextInput style={[styles.input, { flex: 1 }]} value={form.level} onChangeText={(t: string) => setForm({ ...form, level: t })} placeholder="Nível" placeholderTextColor={COLORS.textMuted} accessibilityLabel="Nível" />
            </View>
            <View style={styles.row}>
              <TextInput style={[styles.input, { flex: 1 }]} value={String(form.duration_minutes)} onChangeText={(t: string) => setForm({ ...form, duration_minutes: parseInt(t) || 0 })} placeholder="Duração (min)" keyboardType="numeric" placeholderTextColor={COLORS.textMuted} accessibilityLabel="Duração em minutos" />
              <TextInput style={[styles.input, { flex: 1 }]} value={form.video_id} onChangeText={(t: string) => setForm({ ...form, video_id: t })} placeholder="Video ID YouTube" placeholderTextColor={COLORS.textMuted} accessibilityLabel="ID do vídeo do YouTube" />
            </View>
            <View style={styles.switchRow}>
              <Text style={styles.switchLabel}>Treino Premium (🔒)</Text>
              <Switch
                value={form.is_premium}
                onValueChange={(v: boolean) => setForm({ ...form, is_premium: v })}
