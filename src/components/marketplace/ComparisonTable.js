...
const FIELDS = [
  ...
  { key: 'rating', label: 'Avaliação', format: (v) => v > 0 ? `${v?.toFixed(1)} ★` : 'Sem avaliação' },
  { key: 'brand', label: 'Marca', format: (v) => v || 'N/A' },
  { key: 'category_name', label: 'Categoria', getFrom: (p) => p.marketplace_categories?.name },
  { key: 'description', label: 'Descrição', format: (v) => v ? `${v.slice(0, 100)}...` : 'N/A' },
];

export default function ComparisonTable({ products, visible, onClose }) {
              ...
              <View key={p.id} style={styles.column}>
                ...
                {FIELDS.map(f => (
                  <View key={f.key} style={styles.row}>
                    ...
                    <Text style={styles.value}>{f.getFrom ? f.getFrom(p) : f.format(p[f.key])}</Text>
                  </View>
                ))}
              </View>
            ))}
