...
export default function Marketplace() {
  ...
  const handleSave = async (product) => {
    if (editing) {
      await supabase.from('marketplace_products').update(product).eq('id', editing.id);
    } else {
      await supabase.from('marketplace_products').insert(product);
    }
    setShowModal(false);
    setEditing(null);
    load();
  ...
  const toggleFeatured = async (id, current) => {
    await supabase.from('marketplace_products').update({ is_featured: !current }).eq('id', id);
    load();
  };

  const toggleActive = async (id, current) => {
    await supabase.from('marketplace_products').update({ is_active: !current }).eq('id', id);
    load();
  };
          ...
          <tbody>
            {products.map(p => (
              <tr key={p.id} style={{ borderBottom: '1px solid #222' }}>
                ...
                <td style={{ padding: 8 }}><button onClick={() => toggleFeatured(p.id, p.is_featured)} style={{ cursor: 'pointer', background: 'none', border: 'none', color: p.is_featured ? '#CCFF00' : '#666' }}>{p.is_featured ? '★' : '☆'}</button></td>
                <td style={{ padding: 8 }}><button onClick={() => toggleActive(p.id, p.is_active)} style={{ cursor: 'pointer', background: 'none', border: 'none', color: p.is_active ? '#00E676' : '#FF1744' }}>{p.is_active ? 'Ativo' : 'Inativo'}</button></td>
