"""
Download de modelos MarianMT já quantizados INT8
Fonte: HuggingFace (R4kSo1997)

Modelos disponíveis:
- opus-mt-pt-en-onnx-int8 (~148MB)
- opus-mt-en-pt-onnx-int8 (~348MB)

Uso: python scripts/download_prequantized.py
"""
...
# Modelos pré-quantizados no HuggingFace
MODELS = {
    "pt-en": {
...
def main():
    ...
    for r in all_results:
        ...
        tokenizer = next((f for f in r["files"] if f["file"] == "tokenizer.json"), None)
  ...
  '{r["pair"]}': {{
    ...
    decoderHash: '{decoder["sha256"] if decoder else ""}',
    version: '1.0.0',
  }},""")
